...
z2 = a1 * Theta1';
a2 = [ones(size(a1,1), 1) sigmoid(z2)]; % activation values of hidden layer with bias
h = sigmoid(a2 * Theta2'); % hypothesis

y = [1:num_labels] == y;
J = -sum(sum (( (y .* log(h) ) + ((1 - y) .* ( log(1 - h) ) ) )') ) / m;

% removing bias from thetas
Theta1 = Theta1(:, 2:size(Theta1, 2));
Theta2 = Theta2(:, 2:size(Theta2, 2));
r = ( (sum( sum(Theta1 .* Theta1) ) + sum( sum(Theta2 .* Theta2) )) * ( lambda / (2*m) ) );
J += r;
